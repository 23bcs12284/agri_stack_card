...
import { getCardByIdApi, deleteCardApi, updateCardApi } from '../api/card.api';
...
export const CardDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchCard = async () => {
      ...
      setError('');
      try {
        const response = await getCardByIdApi(parseInt(id));
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await updateCardApi(parseInt(id), {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteCardApi(parseInt(id));
