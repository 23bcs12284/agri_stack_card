...
import { getStatsApi, getUsersApi, deleteUserApi, getAllCardsAdminApi, deleteCardAdminApi } from '@/lib/api/admin.api';
                  ...
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-800">
                      {['Name', 'Email', 'Role', 'Phone', 'Joined', 'Actions'].map((h) => (
                        <th
                          key={h}
                          className="text-left text-[10px] font-bold text-slate-450 uppercase tracking-wider px-5 py-3"
                        >
                          {h}
                        </th>
                      ))}
                    ...
                    <thead>
                      <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
                        {['Order ID', 'Payment ID', 'Amount', 'Status', 'User', 'Date'].map((h) => (
                          <th key={h} className="text-[10px] font-bold text-slate-450 uppercase px-5 py-3">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      {!stats?.activeSessions || stats.activeSessions.length === 0 ? (
                        ...
                        stats.activeSessions.map((s: any) => (
                          <tr key={s.id} className="border-b border-slate-50 dark:border-slate-800/40 hover:bg-slate-50/50 dark:hover:bg-slate-800/10 text-sm">
                            ...
                            <td className="px-5 py-3.5 font-mono text-xs text-slate-650 dark:text-slate-405">{s.deviceId}</td>
